...
class LidarCostFunction: public ceres::SizedCostFunction<1, 3,3,3,3,1,7>
{
    private:
        ...
        const std::vector<std::shared_ptr<std::vector<Pointd> > >& sparse_features_;
    ...
    public:
                ...
                , weight_(weight)
        {
            ...
            {
                feature_times_[i+1] = nanosToSeconds(features_[data_association_.target_ids[i].first]->at(data_association_.target_ids[i].second).t, offset_time);
            }
        }
        ...
        bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
        {
            ...
            Vec3 feature_L = sparse_features_[data_association_.pc_id]->at(data_association_.feature_id).vec3();
            Vec3 feature_I;
