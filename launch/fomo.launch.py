...
def generate_launch_description():
    ...
    ld.add_action(
        DeclareLaunchArgument(
            "bag_path", default_value="", description="Absolute path to a rosbag directory"
        )
    )

    offline_node = Node(
        package="ffastllamaa",
        executable="offline_processor",
        name="ffast2lamaa_offline",
        namespace=NAMESPACE,
        ...
        parameters=[
            {"bag_path": bag_path},
            {"lidar_topic": f"/{LIDAR_TYPE}/points"},
            {"imu_acc_topic": f"/{IMU_TYPE}/data_raw"},
            {"imu_gyr_topic": f"/{IMU_TYPE}/data_raw"},
    ...
    ld.add_action(offline_node)
    ld.add_action(
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=offline_node,
                on_exit=[
                    LogInfo(msg="Offline processor exited; tearing down entire system."),
